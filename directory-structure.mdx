...
training run imports it and calls `load_environment(**params)`. Local
...
**What it is:** held-out evaluation suites for the environment, defined as
`BaseEvalSuite` subclasses returning `EvalCase` rows. It is optional: delete it
...
**When Flash reads it:** [`flash env eval`](/reference/cli#environments) scores
...
**When Flash reads it:** your `environment.py` reads it at run time. Local
`flash train --cost` parses the configured `max_examples` without reading the
dataset, and `--dry-run` checks the config without executing your environment.
...
project = ""   # project uuid from `flash projects list`
model = "Qwen/Qwen3.5-4B"
algorithm = "sft"

[environment]
id = ""   # paste the id returned by `flash env push --project <uuid> --name my-env .`
...
step count for you. Warm-start from a finished SFT run with `init_from_adapter`
for best results.
