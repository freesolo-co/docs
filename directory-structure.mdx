...
<AccordionGroup>
  <Accordion title="environment.py" icon="python">
    ...
    **When Flash reads it:** `flash env push` packages and uploads it; a
    ...
    worker imports it and calls `load_environment(**params)`. The scaffolded
  ...
  <Accordion title="datasets/train.jsonl" icon="table">
    ...
    **When Flash reads it:** your `environment.py` reads it at load time (locally
    for `--dry-run`/`--cost`, and on the worker at run time); `flash env push`
  ...
  <Accordion title="TRAINING.md" icon="book">
    **What it is:** a playbook for the AI coding agent you point at this project —
    ...
    run actually improved the model (not just finished). It's reference for you and
    your agent.

    **When Flash reads it:** never. It's local documentation only; nothing in the
    train, deploy, or chat path depends on it.
