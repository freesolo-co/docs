...
| `algorithm` | string | `sft` | `sft` (supervised): imitates example completions you provide. `grpo` (RL): learns from a reward you define. `opd` (on-policy distillation): a managed teacher model grades your model's own completions. See [Choose a training algorithm](/guides/training#choose-a-training-algorithm). |
...
OPD reuses the GRPO sampling knobs with distillation-tuned defaults, plus a
teacher. It is step-driven (set `steps`, not `epochs`), and warm-starting from a
...
| `teacher_model` | string | `accounts/fireworks/models/glm-5p2` | The distillation teacher. Defaults to the managed GLM-5.2; an override must name a supported teacher. Credentials are platform-managed — you never set a key, and teacher tokens are not billed to you (see [Cost and billing](/reference/cost-model#what-affects-training-cost)). |
