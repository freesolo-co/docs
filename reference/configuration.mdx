...
`GITHUB_TOKEN`, `RUNPOD_API_KEY`, and `RUN_ID` cannot be forwarded.
...
| `init_from_adapter` | string | - | Warm-start from an existing adapter. Must be the full adapter ref `flash status` prints, `<owner>/<repo>:<phase>/<run-id>` (phase is `sft` or `rl`) — a bare run id is rejected. |
...
cheapest validated managed GPU class that fits your run from the platform's
available providers, and a retry walks to the next-cheapest class if one is
unavailable. Do not use `[gpu]` to pin hardware: user-specified `gpu.type` is
not a placement request. Disk, the wall-clock cap, and the auto-retry budget are
platform-managed fields that may appear in resolved specs and run status.
