...
| `algorithm` | string | `sft` | `sft` (supervised): imitates example completions you provide. `grpo` (RL): learns from a reward you define. See [SFT vs GRPO](/guides/training#choose-sft-or-grpo). |
...
| `max_length` | int | recipe default | Max sequence length. Can't exceed the base model's served context window (`max_model_len`); a run whose training context is longer is rejected at submit — SFT checks `max_length`, GRPO checks the rollout prompt plus `max_tokens`. See [context window](/reference/models#context-window-and-lora-rank). |
