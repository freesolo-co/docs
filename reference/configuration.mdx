...
`model` and `environment.id` are required. `algorithm` defaults to `grpo`;
...
| `algorithm` | string | `grpo` | `grpo` (RL): learns from a reward you define. `sft` (supervised): imitates example completions you provide. See [SFT vs GRPO](/guides/training#choose-sft-or-grpo). |
