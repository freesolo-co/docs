...
| `params` | table | `{}` | Keyword arguments passed to the env's `load_environment(**kwargs)`. `split` is also honored by Flash for packaged env datasets — see [Datasets](/guides/datasets#load-sidecars): it selects `datasets/<split>.jsonl`, and a missing split file is an error (no silent `train.jsonl` fallback). |
...
| Key | Type | Default | Description |
| --- | --- | --- | --- |
| `lora_rank` | int | `32` | LoRA rank; higher means a larger adapter with more capacity and cost. Deployment validates the trained adapter against the serving rank cap for the selected base model. |
...
| `init_from_adapter` | string | - | Warm-start from an existing adapter. Must be the full adapter ref `flash status` prints, `<owner>/<repo>:<phase>/<run-id>` (phase is `sft` or `rl`) — a bare run id is rejected. Append `/checkpoints/step-<N>` (a step listed by `flash checkpoints`) to warm-start from that saved checkpoint instead of the run-level adapter, e.g. to continue GRPO from the best step. |
...
cheapest validated managed GPU class that fits your run, and retry uses the next
fitting class when capacity shifts. Disk, the wall-clock cap, the auto-retry
budget, and run artifact storage are platform-managed fields that may appear in
resolved specs and run status.

Run artifacts are written to a private environment-scoped repo with the
content-addressed Flash code snapshot used for that run.
