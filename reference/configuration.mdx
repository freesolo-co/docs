...
| `init_from_adapter`  | string    | -              | Continue a GRPO or OPD adapter from `RUN_ID` or the exact `RUN_ID/step-N` shown by `flash runs checkpoint`. Longer storage refs are rejected. Omit rank and alpha. If set, `model_revision` must match the source exactly.                                                                                                                  |
