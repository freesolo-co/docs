...
completions and has the managed teacher grade them before updating, priced on
GPU time like the others — the teacher runs on Freesolo's managed key, so its
tokens are not billed to you. The estimator sizes the run's memory and prices
the cheapest GPU class that fits — it does not pin a specific card, so the one
actually provisioned (and its live `flash gpus` rate) can differ from the
estimate.
...
- **[Algorithm](/guides/training#choose-a-training-algorithm).** SFT usually costs least. GRPO and OPD both sample and
  ...
  billed to you.
- **Epochs and step caps.** More `epochs` increases cost for SFT, GRPO, and OPD.
  For GRPO/OPD, Flash derives optimizer steps from `epochs`, the retained prompt
  count, and `batch_size`. SFT also supports a `max_steps` cap.
...
For GRPO and OPD, the step count shown in the preview is derived from `epochs`,
the retained prompt count, and the resolved prompts per step.
...
- Run short smoke tests before scaling `epochs` or the retained dataset size.
