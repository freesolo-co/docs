...
Use it before any non-trivial run. It validates the config, estimates the
training loop, prints the quoted USD cost, and exits without submitting.
...
Prefix caching is automatic. If a request shares a leading prompt prefix with a
recent request, the cached prompt-token rate applies to the reused prefix.
Completion tokens always bill at the completion-token rate.
