...
tokens counted from your environment; an OPD step samples the model's completions
and has the managed teacher grade them before updating, priced on GPU time like
the others — the teacher runs on Freesolo's managed key, so its tokens are not
billed to you. The estimator sizes the run's memory and
...
- **[Algorithm](/guides/training#choose-a-training-algorithm).** SFT usually costs least. GRPO and OPD both sample and
  ...
  billed to you.
- **Steps or epochs.** More SFT `epochs`, more GRPO/OPD `steps`, or a higher
  SFT `max_steps` cap increases cost.
