...
The estimator prices the cheapest eligible GPU. `gpu.type` is not a pin. If you
set `[gpu] provider` or `exact_type`, the estimate uses those same constraints,
and they persist across submissions and retries.
