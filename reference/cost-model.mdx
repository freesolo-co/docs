...
per-step time.
Setup before the first training step is reported as wall-clock time but is
**not billed**.
...
The estimator prices the cheapest eligible GPU automatically. If you pin
`[gpu] type`, the estimate uses that same constraint, and it persists across
...
- For thinking runs, raise `max_completion_tokens` only enough for reasoning plus
  the final answer.
- Use `flash runs checkpoint <run-id>` and deploy a good intermediate checkpoint
