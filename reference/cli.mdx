...
<ResponseField name="flash gpus">
  ...
  estimated $/hr. Selection is automatic by default; `[gpu] type` pins an
  exact class.
</ResponseField>
...
<ResponseField name="flash traces export [flags]">
  ...
  records, ready to train on. Each trace becomes an `{"input", "output"}` row,
  the same shape `flash env setup` scaffolds, so the output drops straight into
  an environment's `dataset/train.jsonl`. The conversion runs on the Freesolo
  backend, so a file exported here matches the dashboard's trace export.
...
reads the newest 1000 traces; traces with no usable request/response pair are
skipped and counted in the summary. Writing an existing file needs `--force`.

| Flag             | Default               | Effect                            |
| ---------------- | --------------------- | --------------------------------- |
| `--project ID`   | prompt                | Project id to export from         |
| `-o`, `--output` | `dataset/train.jsonl` | Output JSONL path                 |
