...
<ResponseField name="flash gpus">
  List RunPod-backed managed GPU classes with VRAM and the CLI's current `$/hr`
  snapshot. Submitted runs report the realized GPU and provider in `flash runs`
  and `flash status`.
</ResponseField>
...
<ResponseField name="flash train CONFIG [flags]">
  ...
  A run is billed only when it finishes successfully, for the training-loop GPU
  time it used; setup time is reported but not billed (see
  [Cost & billing](/guides/training#cost-and-billing)); the
...
<ResponseField name="flash status RUN_ID [--logs] [-f]">
  Print a run's full status JSON, including its current or final cost record.
  `--logs` prints current logs before the status JSON. `-f`/`--follow` streams
  logs until the run ends, then prints final status.
</ResponseField>
