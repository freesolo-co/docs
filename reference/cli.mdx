...
If your API key turns up projects with [recorded traces](/guides/tracing), the
survey also offers to seed `dataset/train.jsonl` from one of them instead of the
2 starter rows, and sizes `max_examples` to what it exported. Traces are a head
start, never a requirement: choose "start from scratch", or hit any export
failure, and setup falls back to the starter dataset.
