...
<ResponseField name="flash env push --name NAME [PATH]">
  ...
  `namespace/name`; the namespace is validated server-side and must match your
...
<ResponseField name="flash cancel RUN_ID">
  Cancel a run. The CLI waits for the server to tear down the worker and GPU
  instance before returning, which can take several minutes.
</ResponseField>
...
<ResponseField name="flash checkpoints RUN_ID">
  ...
  line is single-space separated (`step N repo:subfolder`), so it splits
