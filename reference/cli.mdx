...
<ResponseField name="flash gpus">
  List RunPod-backed managed GPU classes with VRAM and a static `$/hr` snapshot.
  Submitted runs report the realized GPU and provider in `flash runs` and
  `flash status`.
</ResponseField>
...
<ResponseField name="flash env pull ENV_ID [PATH] [-o OUTPUT] [-f]">
  ...
  can be a managed slug such as `your-name/your-env`, a
