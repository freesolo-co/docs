...
<AccordionGroup>
  <Accordion title="When am I charged for a run?">
    A run is billed only when it **finishes successfully**, for the
    training-loop GPU time it used. Setup time is reported but not billed, and a
    run that doesn't complete isn't billed for training. Nothing is charged at
...
<Accordion title="Reward collapses to ~0 with `thinking = true`">
  ...
  <Accordion title="A GPU hiccup or unavailability interrupted training">
    ...
    **resumes from the last streamed checkpoint**, and the allocator walks to the
    ...
    does not create a separate training charge; if the run ultimately succeeds,
    the final charge is based on measured training-loop GPU time. Watch with
