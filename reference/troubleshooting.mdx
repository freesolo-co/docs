...
<AccordionGroup>
  ...
  <Accordion title="GRPO pre-flight demands a bigger GPU than I expected">
    The GRPO pre-flight also checks that the rollout engine's KV cache fits: on
    a card sized only for the training peak, a long-context or large
    `group_size` rollout can leave vLLM no room for cache blocks and the worker
    dies at startup ("No available memory for the cache blocks"). Flash now
    catches that shape at submit time and sizes onto a bigger GPU class
    instead. To stay on a smaller (cheaper) card, reduce `max_length`,
    `max_tokens`, or `group_size`.
  </Accordion>
</AccordionGroup>
...
<Accordion title="Reward collapses to ~0 with `thinking = true`">
  ...
  <Accordion title="flash cancel takes a long time to return">
    Expected. Cancellation waits for the server to stop the worker, destroy the
    GPU instance, and clean up the endpoint before confirming, which can take
    several minutes. The CLI waits this teardown out; the run is marked
