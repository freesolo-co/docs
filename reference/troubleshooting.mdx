...
<Accordion title="Qwen3.5 thinking multi-turn SFT learns doubled thinking tags">
  ...
  <Accordion title="OPD run underperforms SFT, or its rollouts never terminate">
    ...
    *toward* its teacher — the managed GLM 5.2 by default, or the one you set with
    `teacher_model` — so if that teacher isn't strong on your specific task,
    distillation copies its mistakes and can land the student *below* SFT — a
    falling per-token loss just means it matched a weak teacher. Before you re-run,
    roll that teacher through your own environment on a held-out split and confirm
    it clearly beats your model *and* solves the task the way you want; if it
    doesn't, this is a job for GRPO or SFT, not OPD — or try a different allow-listed
    `teacher_model`.
