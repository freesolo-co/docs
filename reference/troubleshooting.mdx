...
<Accordion title="flash cancel takes a long time to return">
  ...
  <Accordion title="A capacity or resource-fit issue interrupted training">
    Flash uses bounded retries. SFT, GRPO, and OPD resume from the latest complete
    full-state checkpoint at a clean optimizer-step boundary when one is
    available. OPD may start a replacement fresh only when no earlier attempt
    reached its first optimizer update.

    If an earlier OPD attempt may have updated but its checkpoint is missing,
    incomplete, unreadable, incompatible, or ambiguous, replacement fails closed
    instead of restarting from fresh state.
