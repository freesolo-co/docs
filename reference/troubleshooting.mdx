...
<AccordionGroup>
  ...
  <Accordion title="Run rejected: training context exceeds serving context">
    ...
    checks `train.max_length`; GRPO and OPD check the rollout prompt plus
    `max_tokens`.
    Most models serve at 8192; `Qwen/Qwen3.6-35B-A3B` serves at 4096. Lower
    `max_length` (and, for GRPO, `max_tokens`) to at or below the serving context
...
<AccordionGroup>
  ...
  <Accordion title="Run submit reports insufficient balance">
    ...
    models, fewer GRPO/OPD steps, lower `group_size`, shorter `max_tokens`, and
    SFT smoke tests are the fastest ways to lower the pre-flight estimate.
  </Accordion>
