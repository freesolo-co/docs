...
<Accordion title="Reward collapses to ~0 with `thinking = true`">
  ...
  - Make your reward's parser **thinking-aware**: strip a leading
    `<think>…</think>` block (and tolerate reasoning prose) before extracting the
    answer, so an answer that *did* fit still scores.
