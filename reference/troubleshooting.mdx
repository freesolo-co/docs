...
<Accordion title="Warm-start rank, alpha, or revision mismatch">
  The source LoRA rank and alpha are authoritative, so omit
  `train.lora_rank` when using `init_from_adapter`. `train.lora_alpha` is
  managed (always `2 * lora_rank`) and is never a config key. A
...
<Accordion title="My run is warming up for many minutes">
  ...
  training kernels. `flash log <run-id> -f` prints a `warming up (stage=...)`
