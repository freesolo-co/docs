...
<AccordionGroup>
  ...
  <Accordion title="ModuleNotFoundError: No module named 'freesolo' (local authoring)">
    Training is managed, but local authoring still needs the SDK installed in
    your local Python environment, as does a `flash train --cost` that **loads**
    the environment (for example, to count examples for SFT cost). `--dry-run`
    validates only the config and does not import the environment:
  ...
  <Accordion title="Worker import fails with ModuleNotFoundError">
    ...
    Only list packages your environment imports.
  </Accordion>
  ...
  <Accordion title="[environment].pip breaks training startup">
    ...
    `vllm`, `peft`, `bitsandbytes`, or `liger-kernel`, unless your environment
  ...
  <Accordion title="flash env pull says the archive is too large">
    ...
    flash env pull your-org/your-env datasets/eval.jsonl -o eval.jsonl
    ```
