...
Constrain a response to valid JSON, a fixed choice, or a regex with the
OpenAI-standard `response_format`. A training `structured_outputs` constraint
becomes the deployed adapter's default, and request-level `response_format`
overrides it. This works on adapters and base models:
