...
`timeout_seconds` override, so one slow request gets its own budget (or a tight one) without
changing the client-wide timeout set at construction:
...
- **`X-Freesolo-Checkpoint`** (response): echoes the checkpoint that
  served the request. The header is omitted when the adapter has no
  checkpoint (a plain final-adapter deployment).
- **`X-Freesolo-Expected-Checkpoint`** (request): pins the request to an
  expected checkpoint. If the adapter is serving a different one
  (e.g. a parallel sweep redeployed another step in between), the request fails
  with **409 Conflict** — naming the live checkpoint — instead of
