...
Deployments are OpenAI-compatible. Point any OpenAI SDK at the endpoint from
`flash deployments`. The `model` is your run id. A valid Freesolo API key is
required, and the key's org must own the adapter named in `model`:
...
Constrain a response to valid JSON — or a fixed set of choices, or a regex — with
the OpenAI-standard `response_format`, so the model can't drift off format. It
works on any deployed adapter or base model through the same OpenAI-compatible
client:
...
`{"type": "text"}` (the default) leaves output unconstrained. Guided decoding
binds from the first generated token, so use it with a non-thinking deployment —
or pass `enable_thinking: false` on a base model — when you want strictly
formatted output. See the [Structured outputs](/guides/structured-outputs) guide
to also constrain training rollouts.
