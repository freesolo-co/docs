...
```bash
flash models deploy <run-id>
```

Preview what a deploy would do without creating it:

```bash
flash models deploy <run-id> --dry-run
```
...
```bash
flash models deploy <run-id> --wait
```

It blocks until the revision is servable and exits `0`, or exits `1` if the
...
[dashboard](/platform#models). Both paths run the same verification and produce
...
<Note>
  ...
  adapter, so a plain `flash models deploy <run-id>` fails with an error that lists the
...
serving reuses that prefix from cache instead of recomputing it. Only the
...
```bash
flash models chat <run-id> -m "Summarize the plot of Hamlet in two sentences."
```
...
```bash
flash models chat <run-id> -m "Write a haiku about mountain weather" --temperature 0.8 --max-tokens 128
```
...
```bash
flash models chat <run-id> -m "What is 6*7?" --system "Answer with just the number."
```

## Manage deployments

```bash
flash models deployments         # human-readable deployment status
flash models deployments --json  # complete machine-readable records
flash models undeploy <run-id>   # disable the alias and immutable revisions
```
...
`flash models undeploy <run-id>` disables the stable alias and all of the run's
immutable revisions.

## Use it from your own code

Deployments are OpenAI-compatible. Use `flash models deployments --json` and pass its
...
client = OpenAI(
    base_url="https://serve.freesolo.co/v1",  # openai_base_url from flash models deployments --json
...
When a generation runs in thinking mode, the reasoning comes back in
`reasoning_content` and only the answer in `content`. The `</think>` separator is
...
print(resp.choices[0].message.reasoning_content)  # how it got there
print(resp.choices[0].message.content)  # the answer
```

Streaming splits the same way: reasoning tokens arrive as
...
`flash models chat` folds the two fields back into a single balanced
`<think>...</think>` block, so the CLI shows the reasoning and the answer
together. You only see the split shape when you call the endpoint yourself.

## Send images

Every catalog model accepts **image inputs**, so a served adapter or base model
can take multimodal prompts over the same OpenAI-compatible API. Pass images as
OpenAI `image_url` content parts. Serving accepts base64 `data:` URIs only;
...
To train on image data, see [Image inputs](/guides/datasets#image-inputs).
...
id — any id from `flash models list`, e.g. `Qwen/Qwen3.5-4B` — to generate against the
base weights with no LoRA and no `flash models deploy`:
...
```bash
flash models export --adapter-id <run-id> --repository <owner>/<name>
```
...
<CardGroup cols={2}>
  <Card
    title="Serving prices"
    icon="dollar-sign"
    href="/reference/models#serving-prices"
  >
