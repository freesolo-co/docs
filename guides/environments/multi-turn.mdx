...
This is the full surface Flash expects:
...
By default, multi-turn GRPO assigns **one reward to the whole episode**
(`credit_assignment = "per_episode"`): every assistant turn shares the
trajectory's group-relative advantage. For a pure multi-turn task where each turn
is independently gradable, set `credit_assignment = "per_turn"` in `[train]` to
give **each assistant turn its own group-relative advantage** instead.

Supply the per-turn signal from `score_episode` by returning a `RewardResult`
whose `metadata` carries a `per_turn_rewards` list, one finite value per sampled
assistant turn, in order. Read the turns from `episode.turns`, which holds only
the turns Flash sampled (and any environment replies), never the seeded
`start_episode` prompt, so filtering it to assistant turns matches the graded
count exactly:

```python
def score_episode(self, example, episode) -> RewardResult:
    turns = [t for t in episode.turns if t.role == "assistant"]
    per_turn = [self.grade_turn(t) for t in turns]
    return RewardResult(
        score=sum(per_turn) / len(per_turn),
        metadata={"per_turn_rewards": per_turn},
    )
```

The list length must equal the number of sampled assistant turns and every value
must be finite. If `per_turn_rewards` is absent, Flash silently uses
episode-level credit; if it is supplied but invalid (wrong length, non-numeric,
or non-finite), Flash logs a warning and falls back to episode-level credit for
that rollout. `per_turn` credit is rejected for native tool-calling multi-turn
...
- **`max_completion_tokens`** is _per assistant action_ (one tool call or one
