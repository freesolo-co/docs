...
whose `metadata` carries a `per_turn_rewards` list, one finite value per
assistant turn, in order:

```python
def score_episode(self, example, episode) -> RewardResult:
    per_turn = [self.grade_turn(m) for m in episode.messages if m["role"] == "assistant"]
    return RewardResult(
...
The list length must equal the number of assistant turns and every entry must be
finite. If it is missing, malformed, or the wrong length, Flash logs a warning
and falls back to episode-level credit for that rollout. `per_turn` credit is
rejected for native tool-calling multi-turn environments, and a single-turn run
