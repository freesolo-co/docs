...
Setup needs a project: run `flash login` first, then pass `--project` (from
`flash projects list`) or let an interactive run prompt you to choose one. The
UUID is written into every generated config.
...
needs no local install.** Training runs already have the SDK when they import
