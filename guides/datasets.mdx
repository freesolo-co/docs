...
<Warning>
  Records are canonicalized to `input`/`output`/`id`/`metadata`. Any other
...
Records without an `id` get a deterministic positional one, `example_NNNNNN`
(`example_000000`, `example_000001`, ...), assigned by source position. Local
loading and remote workers derive the same id for the same row, so selecting
examples by id behaves identically in both places. The helper is exported as
`freesolo.datasets.stable_example_id(index)`.
...
dataset = load_dataset("datasets/train.jsonl")
...
<Note>
  ...
  dataset files, `split = "eval"` selects `datasets/eval.jsonl` (or `.json`) as
