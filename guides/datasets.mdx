...
<Warning>
  Any other top-level key (say `initial_state` or `minimal_solutions`) is
  **dropped** before the record reaches a training worker. When that happens,
  the SDK raises a `UserWarning` naming the record, and Flash prints a warning
  naming the dropped keys. To preserve extra task data, nest it under
  `metadata`.
</Warning>
