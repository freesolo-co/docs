...
model generates its own answers from the prompt and learns from your reward, so
`output` is optional there and is read only if your `score_response` uses it as
a reference.
