...
For files, use `.jsonl`, `.json`, `.csv`, or `.txt`. It exposes each record's
`input` as `example.input` and `output` as `example.output`; the original row
stays available as `example.record`.
