...
URI, raw bytes, a PIL image, an `http(s)` URL, or a path relative to the packaged
`dataset/` directory:
...
class VisionEnv(EnvironmentSingleTurn):
    def build_prompt_messages(self, example):
...
which Flash attaches to the prompt for you. Either way, one example may hold up
to **8 images**. Remote `http(s)` URLs are **disabled by default**; set
`FLASH_MULTIMODAL_ALLOW_REMOTE_IMAGES=1` in the worker to allow them, and a
relative path must stay inside `dataset/`.
