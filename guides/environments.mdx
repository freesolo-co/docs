...
tiny `datasets/train.jsonl`, two configs (`configs/rl.toml` for GRPO and
...
itself, so do not pin packages such as `torch`, `trl`, `vllm`, `peft`,
`bitsandbytes`, or `liger-kernel` here unless your environment directly imports
them.
...
running or testing your `environment.py` directly, or `flash train --cost`, which
imports it to count training rows — install it in the same Python
...
```bash
flash env pull your-org/your-env datasets/train.jsonl -o train.jsonl
```
