...
- **`opd`**: on-policy distillation, when a stronger model already does the task.
  ...
  design. Because training only pulls your model *toward* the teacher, **the
  teacher is the ceiling**: pick OPD only when your chosen teacher clearly does
  *your* task better than your model already does, and confirm that first (see the
  ...
  tool-calling ones), and the teacher's credentials are platform-managed for every
  allowed teacher — nothing to set up, and teacher tokens aren't billed to you.
...
<Note>
  **Before an OPD run, check the teacher's quality on your task.** OPD's ceiling is
  the teacher, so run your chosen teacher — GLM 5.2 by default, or the one you set
  with `teacher_model` — through your own environment on a held-out split and
...
# teacher_model = "glm-5.2"          # OPD: managed teacher (glm-5.2 | deepseek-v4-pro | kimi-k2.6)
# structured_outputs = '{"type":"object","properties":{"answer":{"type":"string"}},"required":["answer"]}'  # GRPO/OPD: guided decoding

[wandb]
# project = "my-project"             # optional Weights & Biases logging
```

To force every GRPO or OPD rollout into valid JSON — or a regex or a fixed choice
...
<Note>
  Deploy or [export](/guides/deploy-and-chat#export-to-your-own-huggingface-repo) a
  run you want to keep. Freesolo's managed storage garbage-collects a run's
  checkpoints and final adapter about **7 days** after its last activity when the
  run was never deployed — a deployed run, or one that another run warm-started
  from, is kept. Exporting copies the adapter into a HuggingFace repo you own.
</Note>
