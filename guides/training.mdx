...
At submit time the allocator picks the cheapest validated managed GPU class
that fits your run, and retry uses the next fitting class when capacity shifts.
To see the managed GPU classes, VRAM, and representative `$/hr` snapshot:
...
When a run reaches `done`, its adapter lives in a private environment-scoped
artifact repo with the code snapshot used for the run. Serve it with
