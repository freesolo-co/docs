...
`model_revision` pins the base model to one Hugging Face revision. An empty or
omitted value uses the model's default revision. `seed` controls the
...
- **`opsd`**: on-policy self-distillation (**phase-1, experimental**), when a
  reference solution exists but you want the model to _derive_ its own reasoning
  rather than copy it. The model's own **frozen base** (adapter disabled), shown
  the reference answer, teaches its on-policy rollouts under a forward KL, so
  there is no external teacher and no reward. It needs `thinking = true` on a
  thinking-capable model, a single-turn text-only environment, and a dataset with
  gold completions (like SFT). `group_size` must be `1` and `teacher_model` does
  not apply. See [OPSD-specific](/reference/configuration#opsd-specific).

All four are driven by the same [environment](/guides/environments/overview), and
...
```toml
project = "<project-uuid>"           # required, from `flash projects list`
model = "Qwen/Qwen3.5-4B"
# model_revision = "main"            # optional hugging face branch, tag, or commit
algorithm = "sft"
seed = 42
...
`2 * lora_rank`) and is never a config key. `--dry-run` and a real submit both
resolve the source rank for you; local `--cost` may warn and estimate with
defaults instead.

If you set `model_revision`, the source and target revisions must match exactly.
Use `--dry-run` to resolve and validate the source.
...
an active validated GPU class from `flash gpus`, and that pin also applies to
retries and cost estimates. Storage remains managed.

## Validate before you submit

`--dry-run` runs the same submit-time checks as a real run, including warm-start
resolution, and flags any `[train]` keys your CLI version does not recognize. It
allocates no GPU and charges nothing:
...
```bash
flash runs list                # all your runs: state, cost, model
flash runs status <run-id>     # status JSON, including the cost record
flash runs status <run-id> --follow  # poll status until completion, without replaying logs
flash runs log <run-id>              # print the full log snapshot
flash runs log <run-id> --follow     # stream logs until completion
flash runs cancel <run-id>     # cancel a run
```

## After training

When a run reaches `done`, serve it with
[`flash models deploy`](/guides/deploy-and-chat) and talk to it with `flash models chat`.
