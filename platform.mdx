...
**Models** lists the org's deployed LoRA adapters from managed serving. It shows
the adapter id, base model, serving URL, latest reward when the adapter came from
...
requests against the org that owns the adapter, and unattributed legacy adapters
are not treated as user-authorizable until they are attributed.
...
- Training runs, charged after successful completion at the Flash cost quote.
