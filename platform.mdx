...
updates land on its [changelog](https://platform.freesolo.co/changelog).
...
is scoped to the project you have open. Assign one from the dashboard, with
`flash env push --project`, or with the top-level `project` field in a
[training config](/reference/configuration#top-level). What happens without an
assignment differs by surface: a first `flash env push` files the environment
under your org's **Default** project, a training run with no `project` field
stays ungrouped, and [trace recording](/guides/tracing) always requires an
explicit project id (exports can discover or prompt for one).
