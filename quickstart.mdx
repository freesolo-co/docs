...
- **A Freesolo API key**, created in your dashboard at
  ...
  `--dry-run`, authenticate with it. Local `--cost` does not submit.
...
Every run and environment belongs to a [project](/platform#projects). Create one
and keep its UUID; the next steps need it.

```bash
flash projects create my-project
```
...
`[environment] id` in your config and skip ahead to step 6.
...
evaluations.py            # held-out eval suites for flash env eval
dataset/
...
```bash
flash env push --project <project-uuid> --name starter .
```
...
Validate the config first. `--dry-run` runs the same submit-time checks as a real
run and flags any `[train]` keys your CLI version does not recognize. It
allocates no GPU and incurs no charge:
...
flash runs log <run-id> -f     # stream logs until the run finishes
```
...
serving smoke before the stable run-id alias is activated. You can also deploy
and tear down from the [dashboard](/platform#models).

## Step 9: Chat

```bash
flash models chat <run-id> -m "Hello! What can you do?"
```
...
| `flash projects create` | Create a project and print its UUID          | `flash projects create my-project`                 |
| `flash projects list`   | List your projects and their UUIDs           | `flash projects list`                              |
| `flash env setup`       | Scaffold a starter environment and configs   | `flash env setup --project <uuid>`                 |
| `flash env push`        | Publish an environment and print its id      | `flash env push --project <uuid> --name starter .` |
| `flash env eval`        | Score held-out suites against a deployment   | `flash env eval <run-id>`                          |
...
| `flash runs list`       | List your runs with state, cost, and model   | `flash runs list`                                  |
| `flash runs status`     | Show run status and cost                     | `flash runs status <run-id>`                       |
| `flash runs log`        | Print or follow a run's logs                 | `flash runs log <run-id> -f`                       |
| `flash models deploy`   | Serve a trained adapter                      | `flash models deploy <run-id>`                     |
| `flash models chat`     | Send a message to a deployment               | `flash models chat <run-id> -m "hi"`               |
| `flash models undeploy` | Tear a deployment down                       | `flash models undeploy <run-id>`                   |
