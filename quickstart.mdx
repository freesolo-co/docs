...
<Tip>
  **Already have a training loop?** Skip the hand-editing. Point your coding
  ...
  configs/rl.toml with the base model and [train] settings. My reward is in
...
| `flash train`        | Submit a run and follow its logs           | `flash train configs/rl.toml`          |
| `flash train --cost` | Preview the pre-flight cost, no submit     | `flash train configs/rl.toml --cost`   |
