...
Run an authenticated server-side validation first. `--dry-run` runs the same
submit-time preflights as a real submission, creates a run with `state=dry_run`,
reports client/server `[train]` schema mismatches, and allocates no GPU or charge:
