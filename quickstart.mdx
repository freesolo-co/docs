...
<Note>
  ...
  finishes successfully, for the training-loop GPU time it used. Setup and cold
  start time are reported separately but are not billed; the `--cost` number is
...
charge: serving is
[billed per token](/guides/deploy-and-chat#billing) for
requests you send:
