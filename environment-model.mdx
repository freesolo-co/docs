...
<CardGroup cols={3}>
  ...
  <Card title="A reward" icon="trophy">
    ...
    score is the GRPO signal.
  </Card>
</CardGroup>

The same environment drives **SFT** (learn from gold answers), **GRPO** (learn
from reward scores), **OPD** (practice on the prompts while a managed teacher
scores the model's tokens), and **eval**. Swap one line in the config; the
...
<Steps>
  ...
  <Step title="Your reward scores it">
    `score_response` returns a `RewardResult`. That score is the signal the GRPO
    algorithm learns from. OPD uses the same prompt/interaction loop, but its
    learning signal comes from the managed teacher instead of your reward.
  </Step>
</Steps>
...
<Steps>
  ...
  <Step title="Publish it">
    `flash env push --project <project-uuid> --name my-env .` packages the folder and uploads it to the
  ...
  <Step title="Flash loads it at run time">
    Flash installs your published environment, imports `environment.py`, and
    calls `load_environment(**params)` to drive the run.
  </Step>
</Steps>
