...
In OPD, a **teacher** — a much larger hosted model — grades your model's own
completions token by token, and training pulls your model toward the teacher's
distribution. Freesolo manages the teacher (a Fireworks-hosted GLM model) and its
credentials, so there's nothing to set up and you don't pay for teacher tokens —
